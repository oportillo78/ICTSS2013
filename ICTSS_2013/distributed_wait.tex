...
1. WAIT overhead is less than 1\% in response time and throughput for 480s
sampling (higher  in 30s case).
2. If compared to manual WAIT with upload , WAIT-RPT performs better (I had originally  expected the opposite 
hoping to assess the upload cost independently). Other than the human interaction, the only additional  difference  
was the usage of a browser to do the uploads.
3. If compared to manual WAIT without upload,  the impact of WAIT-RPT is
approximately 50ms (2\%), in response time (including  upload  time) and 1-2\%
in throughput.

1. WAIT overhead is around 2\% in response time and throughput for 480s sampling
(higher  in 30s case).
2. The 480 combinations have similar impacts in response time and throughput (compared against PetStore). 
3. The 30 combinations have similar trends, but bigger impacts (compared against PetStore). The main difference is 
the behavior of the applications, which are reflected in the size of their javacores (pending to explore/compare 
their contents –classes vs. workload-related info-):
	- For PetStores, a javacore is ~1MB; while for Portal is ~5MB.
	- In the 480s case, PetStore uploads (before zipping) ~2MB, while  Portal ~10MB.
	- In the 30s case, PetStore uploads ~20MB, while Portal ~100MB.


]]

\begin{table}[!h]
\caption{Metrics for PetStore - single node}
\label{PetStore1}
\centering
...
PetStore:
480 no impact in response time. In resources, around 1-1.5\% in manual mode,
while around 2-2.5\% in automated mode. Memory increase 3-4\% in manual, around
0.5\%. 
Javacore: Average 1MB, average generation time 1 sec, max
2sec.
+ t-test results!!!

\begin{table}[!h]
\caption{Metrics for Portal - single node}
\label{Portal1}
\centering
\begin{tabular}{p{0.2\textwidth}|p{0.16\textwidth}|p{0.16\textwidth}|p{0.15\textwidth}|p{0.13\textwidth}|p{0.16\textwidth}}
\hline
...
Manual, 480s 			& 0.7\% 	& 0.6\%		& -0.1\%	& 1.13\% 	& 0.9\%\\
Automated, 480s 		& 3.4\%		& 1.0\%		& -2.8\% 	& 0.63\% 	& 4.1\%\\
Manual, 30s 			& 14.9\%	& 5.4\%		& -0.6\% 	& 2.97\% 	& 5.3\%\\
...
Portal:
480: still good, but higher . . . main impact caused by javacore!
Javacore: Average 5MB, average generation time 1sec, max 3 sec
+ t-test results!!!
...
\begin{table}[!h]
\caption{Metrics for PetStore - multi node}
\label{Portal1}
\centering
\begin{tabular}{p{0.2\textwidth}|p{0.16\textwidth}|p{0.16\textwidth}|p{0.15\textwidth}|p{0.13\textwidth}|p{0.16\textwidth}}
\hline
...
T-test analysis: p<0.5 significant
Valid in:
- PetStore 1: Avg. Response Time: Both 30; Max. Response Time: None; Avg.
Throughput: Automated WAIT 30.
- PetStore 2: Also no significant
- Portal 1: Avg. Response Time: Auto-WAIT 480, Both 30; Max. Response Time: None; Avg.
Throughput: Automated WAIT 30.
...
Key message/conclusion: The identification of the root causes of performance issues, relies very heavy on the expert knowledge of the user + requires a lot of different tools. There is no possible to indicate an average time per issue identification, but overall is very challenging/time-consuming.
 ...
 ----------------------------
  ...
  PENDING:
  - Complete the results and related work sections!
  - Finalize the Pair T-Test analysis. - DONE
